...
float CircleBody::rayCast(sf::Vector2f position, sf::Vector2f dir)
{
	...
	{
		//std::cout << sqrt(2 * radius*std::fabs(projectedOnNormal) - std::fabs(projectedOnNormal) * std::fabs(projectedOnNormal)) << std::endl;
		return sqrt(2 * radius * std::fabs(projectedOnNormal) - std::fabs(projectedOnNormal) * std::fabs(projectedOnNormal));
