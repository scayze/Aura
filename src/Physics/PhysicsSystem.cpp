		...
		for (unsigned int j = i + 1; j < bodys.size(); j++)
		{
			...
			if (bodys[i]->getAABB().intersects(bodys[j]->getAABB()))
			{
				...
				//if ( (bodys[i]->getType() ^ bodys[j]->getType()) == (Circle ^ Circle))
				{
					...
					if (! (pushVector.x == 0 && pushVector.y == 0)) collide(bodys[i], bodys[j], pushVector);
				}
				...
				//else if ((bodys[i]->getType() ^ bodys[j]->getType()) == (Circle ^ Edge))
				{
					...
					if (!(pushVector.y == 0 && pushVector.x == 0))
					{
...
sf::Vector2f PhysicsSystem::circleCollide(CircleBody* b1, CircleBody* b2)
{
	...
	if (sumradius*sumradius < xdist*xdist + ydist*ydist) return sf::Vector2f(0,0);
...
sf::Vector2f PhysicsSystem::circleEdgeCollide(CircleBody * circle, EdgeBody * edge)
{
	...
	if (
		...
		//Und das Dot Produkt größer als 0 ist
		...
		)
	{
		...
		//Wenn der auf vP1_P2_leftnorm projektierte vektor größer 0 ist
		...
		if (bt == -1 || bodys[i]->getType() == bt)
		{
			if (wt== -1 || ( bodys[i]->getCollisionType() & wt ) > 0)
			{
				...
				if (dist>0.0f)
				{
					...
					//sf::VertexArray * v = new sf::VertexArray(sf::Points, 2);

					//v->append(pos);
					//v->append(pos + Math::vectorSetMagnitude(direction, dist));
