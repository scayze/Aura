...
HealthBar::HealthBar()
{
	...
	hp.setFont(Resources::getFont("emulogic.ttf"));
	hp.setCharacterSize(16);
	hp.setFillColor(sf::Color(5, 80, 5));
...
void HealthBar::tick()
{
	...
	if (player->getHealthPoints() > player->getMaxHealth() / 2) hp.setFillColor(sf::Color(5, 200, 5));
	else if (player->getHealthPoints() < player->getMaxHealth() / 4) hp.setFillColor(sf::Color(200, 5, 5));
	else hp.setFillColor(sf::Color(255, 150, 5));
}
